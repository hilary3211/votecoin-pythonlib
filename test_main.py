...
def ques():
    ...
    if not is_valid:
        ...
        questions(private_key,address, client,title,question,duration,category)
def main():
    ...
    if not is_valid:
        ...
        vote.votes(escrow_private_key, escrow_address, option_zero_address, option_one_address, client)
...
def TL():
    ...
    if not is_valid:
        ...
        private_key = mnemonic.to_private_key(mnemonics)
        vote.Trustedlist(address,private_key, trusted, untrusted, client)
def vote_del():
    address = str(input('enter address')) 
    private_key = str(input('enter private_key'))
    mnemonics = str(input("Enter  mnemonic (Each word should be separate by whitespace): "))
    ...
    if not is_valid:
        ...
        private_key = mnemonic.to_private_key(mnemonics)
        vote.vote_del(address, private_key, delegate_address, category, delegated_vp)
TL()