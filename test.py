...
def opt_in_to_vote(private_key: str, address: str, client) -> bool:
    ...
    return True
def questions(private_key,address, client,title,question,duration,category):
    ...
    insert(address,title,question,category)
...
def make_vote(sender, key, receiver, amount, comment, client):
    ...
    return txn_id
class votee():
    def Trustedlist(self,address,private_key, trusted, untrusted, client):
    ...
    def vote_del(self,address, private_key, delegate_address, category, delegated_vp):
        self.address = address
        self.private_key = private_key
    ...
    def votes(self,escrow_private_key, escrow_address, option_zero_address, option_one_address,option_two_address, client, votes):#"Vote 0 for zero and vote 1 for one: "
